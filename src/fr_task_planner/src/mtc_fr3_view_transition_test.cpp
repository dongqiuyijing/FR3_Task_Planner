...
int main(int argc, char** argv)
{
  ...
  auto node = rclcpp::Node::make_shared("fr3_mtc_view_transition_test", options);
  ...
  RCLCPP_INFO(node->get_logger(), "STEP 8: one directed view transition. No order search.");
  RCLCPP_INFO(node->get_logger(),
              "Source/target are stop-required inspection endpoints. Hold time is not executed "
              "in plan-only STEP 8.");
  ...
  if (!isRequiredView(source.name) || !isRequiredView(target.name) || source.name == target.name)
  {
    RCLCPP_ERROR(node->get_logger(),
                 "ABORT: source_view and target_view must be distinct members of "
                 "{side_pos_y, side_neg_y, top_circle}. got source='%s' target='%s'",
  ...
  auto param_node = rclcpp::Node::make_shared("fr3_mtc_view_transition_params");
  ...
  if (!before_msg)
  {
    RCLCPP_ERROR(node->get_logger(), "/joint_states missing. STEP 8 FAIL.");
  ...
  {
    RCLCPP_ERROR(node->get_logger(), "CurrentState is all zeros. STEP 8 FAIL.");
    shutdownSpinner(executor, spinner);
    return 1;
  }
  ...
  RCLCPP_INFO(node->get_logger(), "========== STEP 8 PREFLIGHT ==========");
  RCLCPP_INFO(node->get_logger(), "Source view:\n%s", source.name.c_str());
  RCLCPP_INFO(node->get_logger(), "Target view:\n%s", target.name.c_str());
  RCLCPP_INFO(node->get_logger(), "Canonical source TCP:\n%s", formatPose(source.tcp).c_str());
  RCLCPP_INFO(node->get_logger(), "Canonical source object:\n%s", formatPose(source.object).c_str());
  RCLCPP_INFO(node->get_logger(), "Canonical target TCP:\n%s", formatPose(target.tcp).c_str());
  RCLCPP_INFO(node->get_logger(), "Canonical target object:\n%s", formatPose(target.object).c_str());
  ...
  task.setName("FR3 Inspection Transition Task");
  ...
  ompl_source->setTimeout(planning_time);
  const std::string source_stage_name = "MoveTo Source Inspection View " + source.name;
  auto move_source =
      std::make_unique<moveit::task_constructor::stages::MoveTo>(source_stage_name, ompl_source);
  move_source->setGroup(group);
  move_source->setIKFrame(ee_link);
  move_source->setGoal(source.tcp);
  move_source->setTimeout(planning_time);
  task.add(std::move(move_source));

  auto ompl_target =
      std::make_shared<moveit::task_constructor::solvers::PipelinePlanner>(node, kOmplPipeline);
  ompl_target->setTimeout(planning_time);
  const std::string target_stage_name = "MoveTo Target Inspection View " + target.name;
  auto move_target =
      std::make_unique<moveit::task_constructor::stages::MoveTo>(target_stage_name, ompl_target);
  move_target->setGroup(group);
  move_target->setIKFrame(ee_link);
  move_target->setGoal(target.tcp);
  move_target->setTimeout(planning_time);
  task.add(std::move(move_target));
  ...
  RCLCPP_INFO(node->get_logger(),
              ...
              "Lift / Pilz LIN\n↓ Restore Part-Table Collision\n↓ %s / OMPL\n↓ %s / OMPL",
              source_stage_name.c_str(), target_stage_name.c_str());
  ...
  RCLCPP_INFO(node->get_logger(), "attempts requested: %ld", static_cast<long>(max_solutions));
  ...
  if (!plan_result || num_solutions < 1)
  {
    ...
    const bool source_stage_ok = state.str().find(source_stage_name) != std::string::npos &&
                                 failure.str().find(source_stage_name) == std::string::npos;
    if (source_stage_ok)
    {
      RCLCPP_ERROR(node->get_logger(), "SOURCE REACHABLE\nTRANSITION FAILED");
    }
    const std::string cls = classifyTransitionFailure(
        failure.str(), robot_model, group, ee_link, attach_link, object_id, touch_links, source,
        target, actual);
    RCLCPP_ERROR(node->get_logger(), "FAILURE CLASSIFICATION:\n%s", cls.c_str());
    const std::string source_diag =
        diagnosePoseFailure(robot_model, group, ee_link, attach_link, object_id, touch_links,
                            source.tcp, actual, "SOURCE");
    const std::string target_diag =
        diagnosePoseFailure(robot_model, group, ee_link, attach_link, object_id, touch_links,
                            target.tcp, actual, "TARGET");
    RCLCPP_ERROR(node->get_logger(), "Source IK / collision diagnosis:\n%s", source_diag.c_str());
    RCLCPP_ERROR(node->get_logger(), "Target IK exists: %s",
                 target_diag.find("IK FAILURE") != std::string::npos ? "NO" : "YES");
  ...
  const auto* source_sol = findStageSolution(leaves, source_stage_name);
  const auto* target_sol = findStageSolution(leaves, target_stage_name);
  ...
  {
    RCLCPP_ERROR(node->get_logger(),
                 "Need PreGrasp OMPL + Grasp LIN + Lift LIN + Source OMPL + Target OMPL segments");
    shutdownSpinner(executor, spinner);
    return 3;
  }
  ...
  const auto& source_seg = segments[3];
  const auto& target_seg = segments[4];
  ...
  const double attach_lift_disc =
      attach_sol && attach_sol->end() ?
          ...
          1.0;
  const double lift_source_disc = maxJointError(lift_seg.points.back(), source_seg.points.front());
  const double source_target_disc = maxJointError(source_seg.points.back(), target_seg.points.front());
  const double max_disc = std::max(
      pre_grasp_disc,
      std::max(grasp_attach_disc, std::max(attach_lift_disc, std::max(lift_source_disc, source_target_disc))));
  applyJoints(fk_state, source_seg.points.back());
  const Eigen::Isometry3d source_end_tcp = tcpInBase(fk_state, ee_link);
  applyJoints(fk_state, target_seg.points.front());
  const Eigen::Isometry3d target_start_tcp = tcpInBase(fk_state, ee_link);
  double source_target_tcp_pos = 0.0;
  double source_target_tcp_ori = 0.0;
  poseError(source_end_tcp, target_start_tcp, source_target_tcp_pos, source_target_tcp_ori);
  ...
  RCLCPP_INFO(node->get_logger(), "Lift→Source: %.9f rad", lift_source_disc);
  RCLCPP_INFO(node->get_logger(), "Source→Target: %.9f rad", source_target_disc);
  RCLCPP_INFO(node->get_logger(), "Source→Target TCP discontinuity: position=%.6f m orientation=%.6f deg",
              source_target_tcp_pos, source_target_tcp_ori);
  ...
  bool source_attached_start = false;
  bool source_attached_end = false;
  bool target_attached_start = false;
  bool target_attached_end = false;
  bool source_part_table_allowed = true;
  bool target_part_table_allowed = true;
  if (source_sol && source_sol->start() && source_sol->start()->scene())
  {
    source_attached_start = source_sol->start()->scene()->getCurrentState().hasAttachedBody(object_id);
    source_part_table_allowed = acmAllowed(*source_sol->start()->scene(), object_id, table_name);
  }
  if (source_sol && source_sol->end() && source_sol->end()->scene())
  {
    source_attached_end = source_sol->end()->scene()->getCurrentState().hasAttachedBody(object_id);
  }
  if (target_sol && target_sol->start() && target_sol->start()->scene())
  {
    target_attached_start = target_sol->start()->scene()->getCurrentState().hasAttachedBody(object_id);
    target_part_table_allowed = acmAllowed(*target_sol->start()->scene(), object_id, table_name);
  }
  if (target_sol && target_sol->end() && target_sol->end()->scene())
  {
    target_attached_end = target_sol->end()->scene()->getCurrentState().hasAttachedBody(object_id);
  }
  ...
  const size_t source_bound_violations =
      countGroupBoundViolations(fk_state, arm_jmg, source_seg.points);
  const size_t target_bound_violations =
      countGroupBoundViolations(fk_state, arm_jmg, target_seg.points);
  std::vector<ObjectPairContact> source_illegal;
  if (source_sol && source_sol->start() && source_sol->start()->scene())
  {
    source_illegal = illegalOnSegment(*source_sol->start()->scene(), object_id, table_name,
                                      touch_links, source_seg.points);
  }
  std::vector<ObjectPairContact> target_illegal;
  if (target_sol && target_sol->start() && target_sol->start()->scene())
  {
    target_illegal = illegalOnSegment(*target_sol->start()->scene(), object_id, table_name,
                                      touch_links, target_seg.points);
  }

  const double source_joint_len = jointPathLengthL2(source_seg.points);
  const double target_joint_len = jointPathLengthL2(target_seg.points);
  const double source_duration = armSegmentDuration(solution_msg, 3);
  const double target_duration = armSegmentDuration(solution_msg, 4);
  bool terminal_vel_available = false;
  const double terminal_speed = terminalSpeed(solution_msg, 4, terminal_vel_available);

  std::map<std::string, double> joint_delta;
  for (const auto& name : kArmJoints)
  {
    joint_delta[name] = target_seg.points.back().at(name) - source_seg.points.back().at(name);
  }

  const std::string target_ik_diag =
      diagnosePoseFailure(robot_model, group, ee_link, attach_link, object_id, touch_links,
                          target.tcp, source_seg.points.back(), "TARGET");

  RCLCPP_INFO(node->get_logger(), "========== SOURCE VIEW %s ==========", source.name.c_str());
  RCLCPP_INFO(node->get_logger(), "Lift→Source planner: OMPL id='%s'", source_seg.planner_id.c_str());
  RCLCPP_INFO(node->get_logger(), "Attached at source start: %s", source_attached_start ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "Attached at source end: %s", source_attached_end ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "part-table during Lift→Source allowed: %s",
              source_part_table_allowed ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "Source TCP error: position=%.6f m orientation=%.6f deg",
              source_ep.tcp_pos, source_ep.tcp_ori);
  RCLCPP_INFO(node->get_logger(), "Source object error: position=%.6f m orientation=%.6f deg",
              source_ep.obj_pos, source_ep.obj_ori);
  RCLCPP_INFO(node->get_logger(), "Source view-center error: %.6f m", source_ep.view_center_err);
  RCLCPP_INFO(node->get_logger(), "Source normal error: %.6f deg", source_ep.normal_err_deg);
  RCLCPP_INFO(node->get_logger(), "Source canonical-up error: %.6f deg", source_ep.up_err_deg);
  logContacts(node->get_logger(), "Source illegal collisions:", source_illegal);
  RCLCPP_INFO(node->get_logger(), "Source joint-bounds violations: %zu", source_bound_violations);

  RCLCPP_INFO(node->get_logger(), "========== TRANSITION %s → %s ==========", source.name.c_str(),
              target.name.c_str());
  RCLCPP_INFO(node->get_logger(), "Source→Target planner: OMPL id='%s'", target_seg.planner_id.c_str());
  RCLCPP_INFO(node->get_logger(), "Attached at target start: %s", target_attached_start ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "Attached at target end: %s", target_attached_end ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "part-table during Source→Target allowed: %s",
              target_part_table_allowed ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "Target IK exists: %s",
              target_ik_diag.find("IK FAILURE") != std::string::npos ? "NO" : "YES");
  RCLCPP_INFO(node->get_logger(), "Target collision-free: %s",
              target_ik_diag.find("COLLISION") != std::string::npos ? "NO" : "YES");
  RCLCPP_INFO(node->get_logger(), "Trajectory points: %zu", target_seg.points.size());
  RCLCPP_INFO(node->get_logger(), "Predicted motion duration: %.6f s", target_duration);
  RCLCPP_INFO(node->get_logger(), "Joint path length: %.6f rad", target_joint_len);
  RCLCPP_INFO(node->get_logger(), "Planning computation time: %.6f s", planning_computation_s);
  RCLCPP_INFO(node->get_logger(), "Joint delta:");
  for (const auto& name : kArmJoints)
  {
    RCLCPP_INFO(node->get_logger(), "  Δ%s=%.6f rad", name.c_str(), joint_delta.at(name));
  }
  RCLCPP_INFO(node->get_logger(), "Target TCP error: position=%.6f m orientation=%.6f deg",
              target_ep.tcp_pos, target_ep.tcp_ori);
  RCLCPP_INFO(node->get_logger(), "Target object error: position=%.6f m orientation=%.6f deg",
              target_ep.obj_pos, target_ep.obj_ori);
  RCLCPP_INFO(node->get_logger(), "Target view-center error: %.6f m", target_ep.view_center_err);
  RCLCPP_INFO(node->get_logger(), "Target normal error: %.6f deg", target_ep.normal_err_deg);
  RCLCPP_INFO(node->get_logger(), "Target canonical-up error: %.6f deg", target_ep.up_err_deg);
  logContacts(node->get_logger(), "Illegal transition collision states:", target_illegal);
  RCLCPP_INFO(node->get_logger(), "Joint-bounds violations: %zu", target_bound_violations);
  if (terminal_vel_available)
  {
    RCLCPP_INFO(node->get_logger(), "Target terminal joint-speed: %.6f rad/s", terminal_speed);
  }
  else
  {
    RCLCPP_INFO(node->get_logger(), "terminal stop validation unavailable");
  }
  RCLCPP_INFO(node->get_logger(), "T_tcp_object preserved: %s",
              (attach_pos <= kAttachPosTolM && attach_ori <= kAttachOriTolDeg) ? "YES" : "NO");
  RCLCPP_INFO(node->get_logger(), "Lift→Source points=%zu duration=%.6f joint_len=%.6f",
              source_seg.points.size(), source_duration, source_joint_len);

  const bool source_ok =
      source_attached_start && source_attached_end && !source_part_table_allowed &&
      source_ep.tcp_pos <= pos_tol && source_ep.tcp_ori <= ori_tol_deg &&
      source_ep.obj_pos <= pos_tol && source_ep.obj_ori <= ori_tol_deg &&
      source_ep.view_center_err <= view_center_tol && source_ep.normal_err_deg <= ori_tol_deg &&
      source_ep.up_err_deg <= up_tol_deg && source_illegal.empty() && source_bound_violations == 0 &&
      source_seg.planner_id != "LIN";
  const bool target_ok =
      target_attached_start && target_attached_end && !target_part_table_allowed &&
      target_ep.tcp_pos <= pos_tol && target_ep.tcp_ori <= ori_tol_deg &&
      target_ep.obj_pos <= pos_tol && target_ep.obj_ori <= ori_tol_deg &&
      target_ep.view_center_err <= view_center_tol && target_ep.normal_err_deg <= ori_tol_deg &&
      target_ep.up_err_deg <= up_tol_deg && target_illegal.empty() && target_bound_violations == 0 &&
      target_seg.planner_id != "LIN" && source_target_disc <= 1e-4;
  const bool all_ok =
      ...
      attach_illegal.empty() && lift_illegal_all.empty() && restore_ok && source_ok && target_ok;
  ...
  RCLCPP_INFO(node->get_logger(), "STEP 8 edge %s → %s verification: %s", source.name.c_str(),
              target.name.c_str(), all_ok ? "PASS" : "FAIL");

  {
    const std::string yaml_path =
        "/tmp/fr3_step8_" + source.name + "_" + target.name + ".yaml";
    ...
    yaml << "joint_path_length: " << target_joint_len << "\n";
    yaml << "trajectory_point_count: " << target_seg.points.size() << "\n";
    yaml << "planning_computation_time: " << planning_computation_s << "\n";
    yaml << "source_end_joints:\n";
    for (const auto& name : kArmJoints)
    {
      yaml << "  " << name << ": " << source_seg.points.back().at(name) << "\n";
    }
    yaml << "target_end_joints:\n";
    for (const auto& name : kArmJoints)
    {
      yaml << "  " << name << ": " << target_seg.points.back().at(name) << "\n";
    }
    yaml << "source_geometry_error:\n";
    yaml << "  view_center: " << source_ep.view_center_err << "\n";
    yaml << "  normal_deg: " << source_ep.normal_err_deg << "\n";
    yaml << "  up_deg: " << source_ep.up_err_deg << "\n";
    yaml << "target_geometry_error:\n";
    yaml << "  view_center: " << target_ep.view_center_err << "\n";
    yaml << "  normal_deg: " << target_ep.normal_err_deg << "\n";
    yaml << "  up_deg: " << target_ep.up_err_deg << "\n";
    yaml << "max_collision_count: " << (source_illegal.size() + target_illegal.size()) << "\n";
    yaml << "bounds_ok: "
         << ((source_bound_violations + target_bound_violations) == 0 ? "true" : "false") << "\n";
    RCLCPP_INFO(node->get_logger(), "Wrote edge diagnostics: %s", yaml_path.c_str());
  }
