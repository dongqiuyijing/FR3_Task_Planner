...
def _launch_nodes(context, *args, **kwargs):
    ...
    n_top = sum(1 for name in params["roll_views"] if name == "top_circle")
    return [
        LogInfo(
            msg="\n".join(
                [
                    "========== STEP 11B PREFLIGHT ==========",
                    "Diagnosis only. No execute. No ACM/geometry change.",
                    f"top_circle roll poses: {n_top}",
                    "Official validity remains the full PlanningScene.",
                ]
            )
        ),
        Node(
            package="fr_task_planner",
            executable="fr3_mtc_collision_diag",
            name="fr3_mtc_collision_diag",
            output="screen",
            parameters=[
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value="/tmp/fr3_step11b_top_circle.yaml",
            ),
