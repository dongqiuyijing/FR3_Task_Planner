...
from stage4_pregrasp import compute_stage4_pregrasp_params, format_preflight  # noqa: E402


def _launch_nodes(context, *args, **kwargs):
    """Compute Stage4 PreGrasp via fr_control, then start the plan-only node."""
    ...
    return [
        LogInfo(msg=format_preflight(params)),
        Node(
            package="fr_task_planner",
            executable="fr3_mtc_pregrasp_test",
            name="fr3_mtc_pregrasp_test",
            output="screen",
...
def generate_launch_description():
    """Plan-only Home → PreGrasp test. Does not start Stage4 or execute."""
