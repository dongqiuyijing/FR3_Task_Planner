"""STEP 14: optimize Home→PreGrasp→Grasp→Lift without changing frozen A/B/C.

Plan / compare / serialize / RViz only.
No Gazebo execute. No real robot. No execute=true path.
Does not change STEP12C/12D/13 default files or STEP13 trajectory_file.
"""
...
def generate_launch_description():
    ...
    default_traj_out = os.path.expanduser(
        "~/fr_task_ws/src/fr_task_planner/config/step14_optimized_grasp_trajectory.yaml"
    )
    default_diag = os.path.expanduser(
        "~/fr_task_ws/src/fr_task_planner/config/step14_grasp_optimization_diagnostics.yaml"
    )
    return LaunchDescription(
        [
            ...
            LogInfo(
                msg=[
                    "STEP 14 grasp-prefix optimization. execution is forced false. ",
                    "Does not overwrite STEP12C/12D YAML or STEP13 defaults. ",
                    "winner_output=",
            ...
            IncludeLaunchDescription(
                ...
                launch_arguments={
                    ...
                    "step14_search_timeout_sec": LaunchConfiguration("step14_search_timeout_sec"),
                    "step14_prefix_planning_time": LaunchConfiguration(
                        "step14_prefix_planning_time"
                    ),
                    "step14_full_planning_time": LaunchConfiguration("step14_full_planning_time"),
                }.items(),
            ),
        ]
    )
