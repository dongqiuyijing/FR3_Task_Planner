...
"""DUAL-4B-2A: isolate dual-arm handover collision sources.

Read-only. Does not start dual_bringup, apply the scene, or send motion.
"""
...
def _launch_setup(context, *args, **kwargs):
    ...
    node = Node(
        ...
        executable="dual_collision_source_diagnostic",
        name="dual_collision_source_diagnostic",
        output="screen",
        emulate_tty=True,
        parameters=[
            kinematics_params,
    ...
    return [
        LogInfo(msg=["DUAL-4B-2A READ-ONLY collision source diagnostic."]),
        node,
    ]
        ...
        [
            DeclareLaunchArgument(
                ...
                default_value=os.path.join(
                    home, "fr_task_ws/src/fr_task_planner/config/dual_handover.yaml"
                ),
            ),
            DeclareLaunchArgument(
                "kinematics_yaml",
