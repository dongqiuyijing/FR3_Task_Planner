#!/usr/bin/env python3
"""RViz preview of only A Handover -> Home. Not executable."""
...
def generate_launch_description():
    ...
    if not os.path.isfile(rviz):
        rviz = os.path.expanduser("~/fr_task_ws/src/fr_task_planner/config/keypose_b_face45.rviz")
    robot = {
    ...
    return LaunchDescription(
        [
            LogInfo(msg="A HANDOVER TO HOME PREVIEW ONLY. Does not move the robot."),
            Node(
                package="fr_task_planner",
                executable="keypose_v1_a_return_preview.py",
                name="keypose_v1_a_return_preview",
                output="screen",
            ),
            Node(
                package="rviz2",
                executable="rviz2",
                name="keypose_v1_a_return_rviz",
                output="screen",
                arguments=["-d", rviz],
                parameters=[robot],
            ),
