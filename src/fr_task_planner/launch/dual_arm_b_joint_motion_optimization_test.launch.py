...
"""DUAL-6B: Arm B H→I1→I2→I3 joint-motion geometric optimization. PLAN ONLY.
...
Does not perform TOTG / IPTP / Ruckig.
"""
...
def _launch_setup(context, *args, **kwargs):
    ...
    optimization_yaml = _existing(
        ...
        os.path.join(planner_share, "config", "dual_arm_b_joint_motion_optimization.yaml"),
        os.path.join(
            home,
            "fr_task_ws/src/fr_task_planner/config/dual_arm_b_joint_motion_optimization.yaml",
        ),
    )
    ...
    node = Node(
        ...
        executable="dual_arm_b_joint_motion_optimization_test",
        name="dual_arm_b_joint_motion_optimization_test",
        output="screen",
        emulate_tty=True,
        parameters=params,
    )
    ...
    return [
        LogInfo(
            msg=[
                "DUAL-6B ARM B JOINT-MOTION GEOMETRIC OPTIMIZATION ONLY. "
                "Will not start dual_bringup, apply the scene, or send motion. "
                "Time parameterization will not be performed."
            ]
        ),
        ...
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value=os.path.join(
                    home,
                    "fr_task_ws/src/fr_task_planner/config/"
                    "dual_arm_b_joint_motion_optimization.yaml",
                ),
            ),
            DeclareLaunchArgument(
                "kinematics_yaml",
